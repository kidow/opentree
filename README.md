...
`opentree vercel status`는 현재 local project link, Vercel CLI 설치 여부, 로그인 상태를 읽기 전용으로 보여준다.
`opentree vercel status --json`은 같은 상태 정보를 JSON으로 출력한다.
