...
describe("PhonePreview", () => {
  it("renders profile name + bio when profile block is enabled", () => {
    render(<PhonePreview config={makeConfig()} />);
  ...
  it("hides disabled blocks", () => {
    ...
    render(<PhonePreview config={c} />);
    expect(screen.queryByText("GitHub")).not.toBeInTheDocument();
  });

  it("renders avatar img when avatarUrl is set, placeholder otherwise", () => {
    const { container, rerender } = render(<PhonePreview config={makeConfig()} />);
    expect(container.querySelector(".preview-avatar-placeholder")).not.toBeNull();

    rerender(<PhonePreview config={makeConfig({ profile: { name: "Alice", avatarUrl: "https://x/a.png" } })} />);
    expect(container.querySelector("img.preview-avatar")).not.toBeNull();
  });
  ...
  it("applies theme background/accent colors via inline styles", () => {
    ...
    const { container } = render(<PhonePreview config={c} />);
    const screen_ = container.querySelector(".phone-screen") as HTMLElement;
    expect(screen_.style.background).toMatch(/rgb\(0,\s*255,\s*0\)|#00ff00/i);
    const link = container.querySelector(".preview-link") as HTMLElement;
  ...
  it("renders nothing for unsupported block types (e.g. socials)", () => {
    ...
    const { container } = render(<PhonePreview config={c} />);
    // no preview-link / preview-heading / preview-text rendered
    expect(container.querySelector(".preview-link")).toBeNull();
    expect(container.querySelector(".preview-heading")).toBeNull();
  });
});
